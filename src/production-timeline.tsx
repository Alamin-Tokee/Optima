      ...
      {timeSlots.map((slot) => (
        <div key={slot.hour} className="flex items-stretch h-8">
          ...
          <div className="w-16 flex items-center justify-end pl-2 text-sm tabular-nums">
            {slot.hour === 10 && <span className="text-red-500">31.22/250</span>}
            {slot.hour === 11 && <span className="text-green-500">281.79/221.81</span>}
            {(slot.hour === 12 || slot.hour === 13) && <span className="text-red-500">0/0</span>}
          </div>
        </div>
      ))}
    </div>
